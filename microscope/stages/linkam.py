...
class _LinkamBase(microscope.abc.FloatingDeviceMixin, microscope.abc.Device):
    ...
    def init_sdk():
        ...
        cfunc = ctypes.CFUNCTYPE(None, _CommsHandle, _uint32_t)(
            __class__._on_error
        )
    ...
    def _on_error(cls, h: _CommsHandle, errcode: _uint32_t):
        ...
        if not stage:
            return
        if err in (
            ErrorCode.USBCommsTxError,
            ErrorCode.USBCommsRxError,
            ErrorCode.SerialCommsTxError,
            ErrorCode.SerialCommsRxError,
        ):
    ...
    def _process_msg(
        self, msg, param1=None, param2=None, param3=None, result=None
    ):
    ...
    def set_value(self, svt, val):
        ...
        return self._process_msg(
            Msg.SetValue, _StageValueType(svt).value, _Variant(**{vtype: val})
        ).vBoolean
    ...
    def init_usb(self, uid):
        ...
        if uid is None:
            ...
            uid = uid.encode()
        self._lib.linkamInitialiseUSBCommsInfo(
            byref(self._commsinfo), ctypes.c_char_p(uid)
        )
    ...
    def get_status(self, *args):
        ...
        for s in structs:
            names = filter(
                lambda n: not n.startswith("unused"),
                (f[0] for f in s.flags._fields_),
            )
            status.update(
                dict(map(lambda n: (n, bool(getattr(s.flags, n))), names))
            )
        return status
...
class _LinkamMDSMixin:
    ...
    def _post_connect(self):
        ...
        for name, flag, svt in (
            (
                "X_velocity",
                self._stageconfig.flags.motorX,
                _StageValueType.MotorVelX,
            ),
            (
                "Y_velocity",
                self._stageconfig.flags.motorY,
                _StageValueType.MotorVelY,
            ),
            (
                "Z_velocity",
                self._stageconfig.flags.motorZ,
                _StageValueType.MotorVelZ,
            ),
        ):
            if flag:
    ...
    def _update_status(self, status):
        ...
        self.get_value(
            _StageValueType.MotorDrivenStageStatus, result=self._mdsstatus
        )
...
class LinkamCMS(_LinkamMDSMixin, _LinkamBase):
    ...
    _refill_map = {
        "sample": "sampleDewarFillSignal",
        "external": "mainDewarFillSignal",
    }
    _heater_map = {
        "t_bridge": _StageValueType.Heater1Temp,
    ...
    class RefillTracker:
        ...
        def as_dict(self):
            ...
            return dict(
                refilling=self.refilling, last=self.t, between_last=self.dt
            )

        def __repr__(self):
            """Display tracker properties in representation."""
            return "refilling: %s, t: %s, dt: %s" % (
                self.refilling,
                self.t,
                self.dt,
            )
    ...
    def __init__(self, uid="", **kwargs):
        ...
        # Refill tracking
        self._refills = dict(
            {k: self.RefillTracker() for k in self._refill_map}
        )
    ...
    def temperatures(self):
        ...
        return dict(
            (key, self.get_value(svt)) for key, svt in self._heater_map.items()
        )
