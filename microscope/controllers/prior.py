...
class _ProScanIIIConnection:
    ...
    def is_limit_switch_active(self, axis: str, sign: int) -> bool:
        """Reports whether specific limit switch is currently active.
...
class _ProScanIIIStageAxis(microscope.devices.StageAxis):
    ...
    def move_by(self, delta: float) -> None:
        if self._name == 'X':
            ...
            raise RuntimeError('we should never have gotten here')
...
class _ProScanIIIStage(microscope.devices.StageDevice):
    ...
    @property
    def position(self) -> typing.Dict[str, float]:
...
class ProScanIII(microscope.devices.ControllerDevice):
    ...
    `stage`
      XY stage.  The stage axes are named `x` and `y`.
    ...
    .. note::
       ...
       ``stage.enable()`` will move the stage to its physical limits.
       If the stage has not been enabled, then it won't be able to
       report its limits and may not function properly.
    """
